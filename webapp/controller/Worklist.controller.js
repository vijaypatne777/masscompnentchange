	...
	return BaseController.extend("com.mass.compchange.zmasscomponentchange.controller.Worklist", {
		...
		onInit: function () {
			...
			oViewModel = new JSONModel({
				...
				bomMaterialReplTableCount:0,
				bomMaterialDellTableCount:0,
			...
			var oTableRepComp = this.byId("tableCompReplace");
			oTableRepComp.attachEventOnce("onUpdateFinishedRepComp", function () {
				// Restore original busy indicator delay for worklist's table
				oViewModel.setProperty("/tableBusyDelay", iOriginalBusyDelay);
			});
			var oTableDelTable = this.byId("tableDel");
			oTableDelTable.attachEventOnce("onUpdateFinishedRepComp", function () {
				// Restore original busy indicator delay for worklist's table
				oViewModel.setProperty("/tableBusyDelay", iOriginalBusyDelay);
			});
		...
		onUpdateFinishedRepComp: function (oEvent) {
			...
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				...
				this.getModel("worklistView").setProperty("/worklistTableTitle", sTitle);
			this.getModel("worklistView").setProperty("/bomMaterialReplTableCount", [iTotalItems]);
		

		},
		onUpdateFinishedDelComp: function (oEvent) {
			...
			// the table is not empty
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				sTitle = this.getResourceBundle().getText("worklistTableTitleCountDelComp", [iTotalItems]);
			} else {
				sTitle = this.getResourceBundle().getText("worklistTableTitleBOM");	
				}
					this.getModel("worklistView").setProperty("/worklistTableTitle", sTitle);
			this.getModel("worklistView").setProperty("/bomMaterialDellTableCount", [iTotalItems]);
		...
		onSearchRepComp: function (oEvent) {
			...
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				this._applySearchRepComp(aTableSearchState);
			}
		},

		

		onSearchDelComp: function (oEvent) {
			...
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				this._applySearchDelComp(aTableSearchState);
			}
		...
		_applySearchRepComp: function (aTableSearchState) {
		...
		_applySearchDelComp: function (aTableSearchState) {
