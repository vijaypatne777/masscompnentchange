sap.ui.define([
	...
	return BaseController.extend("com.mass.compchange.zmasscomponentchange.controller.Worklist", {
			...
			oModel.submitBatch(
				function (oRecData, oRecResponse, aErrorResponses) { // request successfully sent
					...
					if (oRecResponse.body.search("error") > 0) {
						var erroeMsg = " Error while updating the record.";
		...
		onDeleteBOMComp: function (oEvent) {
			...
			this.oSelectedCompSet = [];
			for (var index = 0; index < selectedItems.length; index++) {
				 selectedRowIndexArr.push(oTable.getSelectedContexts()[index].getPath().substring(9));
			}
		...
		for (var indexToDelete = 0; indexToDelete < oDelTableLength; indexToDelete++) {
				...
				oDialog.open();
				//this.getView().setModel(this.jModelDelTblselectedItem, "itemModelDelRev");
		...
		onOkDeleteComp: function (oEvent) {
			...
			oModel.submitBatch(
				function (oRecData, oRecResponse, aErrorResponses) { // request successfully sent
					...
					if (oRecResponse.body.search("error") > 0) {
						var erroeMsg = " Error while deleting the record.";
						sap.m.MessageBox.show(
							erroeMsg, {
								icon: sap.m.MessageBox.Icon.ERROR,
								title: "Error",
