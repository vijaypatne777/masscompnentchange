	...
	return BaseController.extend("com.mass.compchange.zmasscomponentchange.controller.Worklist", {
			...
			if (oMaterial.length === 0 || oPlant.length === 0) {
				...
				oModel.read("/MaterialBOMItem", { // Get all valid material
					...
					success: function (oData, response) {
						...
						that.jModelDelTblselectedItem.setData(oData);
		...
		onDeleteBOMComp: function (oEvent) {
			...
			if (selectedItems.length > 0) {
				...
				//var oDelTableLength = this.jModelDelTblselectedItem.getData().results.length;
				//alert(" Selected -  Length = " + selectedItems.length);
				//this.jModelDelTblselectedItem.setData().results.length = selectedItems.length;
				//this.jModelDelTbleFinal.results = new sap.ui.model.json.JSONModel([]);

				//var oModelDelTbl = new sap.ui.model.json.JSONModel(this.jModelDelTblselectedItem);
				//oModelDelTbl.setData(this.jModelDelTblselectedItem);
				//jsonModel.getProperty("/results").push
				//oModelelTbl.results[0].material = "Sandip";

				//this.jModelDelTblselectedItem.getData().results.splice(selectedItems.length,oDelTableLength);
				/*this.jModelDelTblselectedItem.getProperty("/results")[0].Material =1;
	this.jModelDelTblselectedItem.getProperty("/results")[0].BillOfMaterial =2;
	this.jModelDelTblselectedItem.getProperty("/results")[0].BillOfMaterialComponent =3;
	this.jModelDelTblselectedItem.getProperty("/results")[0].BillOfMaterialItemQuantity =4;*/
				...
				for (var index = 0; index < selectedItems.length; index++) {
					...
					var oItemData = oTable.getSelectedContexts()[0].getModel().getData().results[selectedRowIndex];
					this.oSelectedCompSet.push(oItemData);
					//oModelDelTbl.getData().getProperty("/results")[index] = this.oSelectedCompSet[index];

					/*this.jModelDelTblselectedItem.getProperty("/results")[index].Material =this.oSelectedCompSet[index].Material;
				 this.jModelDelTblselectedItem.getProperty("/results")[index].BillOfMaterial =this.oSelectedCompSet[index].BillOfMaterial;
				 this.jModelDelTblselectedItem.getProperty("/results")[index].BillOfMaterialComponent =this.oSelectedCompSet[index].BillOfMaterialComponent;
				 this.jModelDelTblselectedItem.getProperty("/results")[index].BillOfMaterialItemQuantity =this.oSelectedCompSet[index].BillOfMaterialItemQuantity;
				*/
					/*this.jModelDelTblselectedItem.getData().results[index].Material =this.oSelectedCompSet[index].Material;
									 this.jModelDelTblselectedItem.getData().results[index].Material =this.oSelectedCompSet[index].Material;*/
				}
				/*sap.ui.getCore().setModel(oModelelTbl.results,this.oSelectedCompSet);
				oModelelTbl.results.setData(this.oSelectedCompSe);*/
				/*this.jModelDelTblselectedItem.getProperty("/results")[0] = 	this.oSelectedCompSet[0];
				this.jModelDelTblselectedItem.getProperty("/results")[1] = 	this.oSelectedCompSet[1];
				this.jModelDelTblselectedItem.getProperty("/results")[2] = 	this.oSelectedCompSet[2];*/
				//oModelDelTbl.getData().getProperty("/results").splice(selectedItems.length, oDelTableLength);

				//var ss = "Sandip";

				/*for (var indexToDelete = 0; indexToDelete < oDelTableLength; indexToDelete++) {
						var oMatchedIndex = selectedRowIndexArr.indexOf(indexToDelete.toString());
						if(oMatchedIndex < 0){
							this.jModelDelTblselectedItem.getData().results.splice(indexToDelete, 1);
						}
					}*/

				oDialog.open();
				//this.getView().setModel(oModelDelTbl, "itemModelDelRev");
